...
function isTSExportableDeclaration(node) {
  return (
    ...
    _core().types.isTSTypeAliasDeclaration(node) ||
    _core().types.isTSModuleDeclaration(node) ||
    (_core().types.isVariableDeclaration(node) && node.declare) ||
    (_core().types.isClassDeclaration(node) && node.declare) ||
    _core().types.isTSDeclareFunction(node)
  );
}
        ...
        Program(path, state) {
          ...
          for (const stmt of path.get('body')) {
            if (isTSExportableDeclaration(stmt.node)) {
              ...
              _core().types.isExportNamedDeclaration(stmt.node) &&
